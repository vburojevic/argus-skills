...
Each answer opens with a `headline`: one sentence of at most 14 words, the one thing to know. It is the whole line Argus's own teaser, widgets, watch and menu bar show, so lead with it when you summarize. Behind it sits the `lead`, usually one 40–60 word paragraph (shorter when facts are sparse), and up to eight `notes`. Notes can be absent when they add no context beyond the lead. The headline is never the lead's first sentence repeated — say one or the other, not both.
...
A null `lead` always carries a `reason`. `nothing_to_say` means the day holds no facts worth a note — or, under `--cached`, that none was stored; `unavailable` means generation could not complete; `rate_limited` means the 24-hour generation ceiling was reached; `generating` means a generation is still active. Report the reason as it stands and never write a replacement brief of your own. `--cached` (`mode=cached`) answers from storage and never waits on generation — use it whenever a slow answer would be worse than no answer.
...
A cold note first reads its fact sheet, then writes out of band. `GET /v1/brief` returns the previous note or `lead: null` with `reason: "generating"` while that work remains active. Pass `wait=1` (the CLI does unless `--cached`) to wait within a 90-second request window; it can still return `generating`, so read again later without inventing a replacement note. Preparation and transport failures remain errors; they are not proof that a note is being written. `mode=cached` only reads storage.

Skills version 1.4.3 · `argus skills check` · update with `argus skills update` or `npx skills add vburojevic/argus-skills`.
