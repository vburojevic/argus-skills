...
A null `lead` carries a `reason`: `nothing_to_say`, `unavailable`, `rate_limited` when the 24-hour generation ceiling is reached, or `generating` while a generation is active; every one of them exits 0 and is reported as it stands, never replaced with commentary of your own.

## Chat

`argus chat "how did I sleep this week"` streams Argus's grounded answer. Use `--thread cht_…` to continue a thread and `--json` for raw event JSONL; `argus chat threads --json` lists recent threads. Use `--question inv_… --revision 2` to start from an exact saved question revision; this is mutually exclusive with `--thread`. At a TTY, `argus chat` opens a REPL. Writes arrive as proposals and still require Apply or Dismiss in a product surface.
