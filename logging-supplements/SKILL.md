...
The connected iPhone/iPad mirrors an intake's stored nutrient projections through the shared Argus-authored mirror, including an intake applied from chat or recorded on any other surface. It uses the intake's original accepted nutrient values, not a later edit of the supplement definition. Undo removes the same mirror IDs on the next successful phone reconciliation. Ingredients without HealthKit quantity types remain statements in Argus. Permission denial or an offline phone can defer the mirror; an Argus receipt alone does not prove an Apple Health save. Never write a second copy from an agent.
