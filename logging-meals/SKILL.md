...
The server parses the text (LLM), checks the parsed items against your words, and chooses a corpus food only when the match is credible. A doubtful match stays `estimated` with the parser's approximate nutrition. Meal nutrients **automatically project to `dietary_*` metrics** — after logging, `argus summary` already reflects them. The response's `items[].source` says `grounded` (matched to a real food), `estimated` (model estimate — nutrition is approximate), or `manual` (nutrition you supplied on the line and Argus stored verbatim; surfaces label it "as stated"). Those three are the whole set. A grounded item's confidence describes the match, not the precision of its portion or nutrient data.
