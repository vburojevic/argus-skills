...
- **The Daily Brief.** Today's Brief states what is new since yesterday, so today's dose counts are not in it — Argus's own Today strip counts what is due, live. A day review may state a closed day's counts in the Today surface's grammar — "1 of 2 medication doses taken" — as a fact or leave them out. The denominator counts dose events (taken and skipped, plus scheduled doses not logged), so a taken as-needed dose earns the line too; a day with no dose events carries none. The Brief never instructs and never reads a count as adherence.
...
### Apple Health write restriction

Verified against the iOS 26.5 SDK: `HKMedicationDoseEvent` has read-only properties and no public initializer or save factory. Argus cannot create an Apple Health medication dose event through the supported API. A confirmed dose stays in Argus; do not substitute a symptom, nutrition sample, or private URL scheme. The general Apple Health mirror does not change this restriction.
