...
Timezone setup records confirmation, never intent inferred from a value. `get_profile` / `argus profile get` returns `timezone_confirmed`; false means no confirmation is recorded, even when timezone is UTC or a legacy non-UTC value. Do not guess from device location or the timezone string.

When the person confirms their timezone, use `argus profile set --timezone UTC --confirm-timezone` (substitute their IANA timezone). MCP tool `update_profile` takes `{"timezone":"UTC","timezone_confirmed":true}`. True requires timezone in the same PATCH so a concurrent change cannot confirm a different value. Without the marker, changing timezone clears confirmation; resending the same value or patching characteristics/units preserves it. `timezone_confirmed:false` explicitly clears confirmation. Legacy profiles retain their timezone and start false because older forms resent untouched timezone fields. Restoring a saved profile must preserve its marker, including false; a legacy snapshot with no marker must not be treated as confirmed. Confirmation changes no ledger timestamps or stored dates.
...
The Argus MCP server lives at `<ARGUS_API_URL>/mcp`. It publishes `argus://brief/today`, the manifest at `argus://skills`, and each current skill at `argus://skills/<name>`. Today's Brief resource reads the insight — label and figure, headline, sentence — then one line per also-new item; a pending or quiet day has empty text, and `_meta` carries its `status`, `reason` and `dismissed_at` instead of invented commentary. Two auth modes:
